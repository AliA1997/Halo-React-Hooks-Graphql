...
const Navbar = ({history}) => {
    ...
    return (
        <div className="navbar" style={{display: !(history.location.pathname === '/') || state.currentUser ? 'flex' : 'none'}}>
            {utils.Icon('nav', handleClick)}
            <ul style={{display: showNavbar ? 'none' : 'initial'}}>
                <li>
                    {
                        state.currentUser ?
